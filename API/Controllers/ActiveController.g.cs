...
{
    /// <summary>
    /// Controller responsible for managing active related operations.
    /// </summary>
    /// <remarks>
    /// This Controller provides endpoints for adding, retrieving, updating, and deleting active information.
    /// </remarks>
    [Route("api/dynamic/active")]
    [Authorize]
    public class ActiveController : BaseApiController
    {
        private readonly IActiveService _activeService;

        /// <summary>
        /// Initializes a new instance of the ActiveController class with the specified context.
        /// </summary>
        /// <param name="iactiveservice">The iactiveservice to be used by the controller.</param>
        public ActiveController(IActiveService iactiveservice)
        {
            _activeService = iactiveservice;
        }
        ...
        public IActionResult Post([FromBody] Active model)
        {
            ...
            var id = _activeService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _activeService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _activeService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _activeService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Active updatedEntity)
        {
            ...
            var status = _activeService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Active> updatedEntity)
        {
            ...
            var status = _activeService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}