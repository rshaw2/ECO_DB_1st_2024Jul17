...
namespace ECODB1st2024Jul17.Controllers
{
    ...
    /// </remarks>
    [Route("api/dynamic/appointmentservice")]
    [Authorize]
    public class AppointmentServiceController : BaseApiController
    {
        ...
        public IActionResult Post([FromBody] Entities.AppointmentService model)
        {
            model.TenantId = TenantId;
            var id = _appointmentServiceService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _appointmentServiceService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _appointmentServiceService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _appointmentServiceService.Delete(id);
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [Produces("application/json")]
        public IActionResult UpdateById(Guid id, [FromBody] Entities.AppointmentService updatedEntity)
        {
            ...
            var status = _appointmentServiceService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [Produces("application/json")]
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Entities.AppointmentService> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = _appointmentServiceService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}