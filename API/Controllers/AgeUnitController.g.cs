...
namespace ECODB1st2024Jul17.Controllers
{
    ...
    public class AgeUnitController : BaseApiController
    {
        ...
        public IActionResult Post([FromBody] AgeUnit model)
        {
            model.TenantId = TenantId;
            model.CreatedBy = UserId;
            model.CreatedOn = DateTime.UtcNow;
            var id = _ageUnitService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _ageUnitService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _ageUnitService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _ageUnitService.Delete(id);
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [Produces("application/json")]
        public IActionResult UpdateById(Guid id, [FromBody] AgeUnit updatedEntity)
        {
            ...
            var status = _ageUnitService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [Produces("application/json")]
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<AgeUnit> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = _ageUnitService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}