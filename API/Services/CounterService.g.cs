using ECODB1st2024Jul17.Models;
using ECODB1st2024Jul17.Data;
using ECODB1st2024Jul17.Filter;
using ECODB1st2024Jul17.Entities;
using ECODB1st2024Jul17.Logger;
using Microsoft.AspNetCore.JsonPatch;
using System.Linq.Expressions;

namespace ECODB1st2024Jul17.Services
{
    /// <summary>
    /// The counterService responsible for managing counter related operations.
    /// </summary>
    /// <remarks>
    /// This service for adding, retrieving, updating, and deleting counter information.
    /// </remarks>
    public interface ICounterService
    {
        /// <summary>Retrieves a specific counter by its primary key</summary>
        /// <param name="id">The primary key of the counter</param>
        /// <returns>The counter data</returns>
        Counter GetById(Guid id);

        /// <summary>Retrieves a list of counters based on specified filters</summary>
        /// <param name="filters">The filter criteria in JSON format. Use the following format: [{"PropertyName": "PropertyName", "Operator": "Equal", "Value": "FilterValue"}] </param>
        /// <param name="searchTerm">To searching data.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <param name="pageSize">The page size.</param>
        /// <param name="sortField">The entity's field name to sort.</param>
        /// <param name="sortOrder">The sort order asc or desc.</param>
        /// <returns>The filtered list of counters</returns>
        List<Counter> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc");

        /// <summary>Adds a new counter</summary>
        /// <param name="model">The counter data to be added</param>
        /// <returns>The result of the operation</returns>
        Guid Create(Counter model);

        /// <summary>Updates a specific counter by its primary key</summary>
        /// <param name="id">The primary key of the counter</param>
        /// <param name="updatedEntity">The counter data to be updated</param>
        /// <returns>The result of the operation</returns>
        bool Update(Guid id, Counter updatedEntity);

        /// <summary>Updates a specific counter by its primary key</summary>
        /// <param name="id">The primary key of the counter</param>
        /// <param name="updatedEntity">The counter data to be updated</param>
        /// <returns>The result of the operation</returns>
        bool Patch(Guid id, JsonPatchDocument<Counter> updatedEntity);

        /// <summary>Deletes a specific counter by its primary key</summary>
        /// <param name="id">The primary key of the counter</param>
        /// <returns>The result of the operation</returns>
        bool Delete(Guid id);
    }

    /// <summary>
    /// The counterService responsible for managing counter related operations.
    /// </summary>
    /// <remarks>
    /// This service for adding, retrieving, updating, and deleting counter information.
    /// </remarks>
    public class CounterService : ICounterService
    {
        private ECODB1st2024Jul17Context _dbContext;

        /// <summary>
        /// Initializes a new instance of the Counter class.
        /// </summary>
        /// <param name="dbContext">dbContext value to set.</param>
        public CounterService(ECODB1st2024Jul17Context dbContext)
        {
            _dbContext = dbContext;
        }

        /// <summary>Retrieves a specific counter by its primary key</summary>
        /// <param name="id">The primary key of the counter</param>
        /// <returns>The counter data</returns>
        public Counter GetById(Guid id)
        {
            var entityData = _dbContext.Counter.FirstOrDefault(entity => entity.Id == id);
            return entityData;
        }

        /// <summary>Retrieves a list of counters based on specified filters</summary>
        /// <param name="filters">The filter criteria in JSON format. Use the following format: [{"PropertyName": "PropertyName", "Operator": "Equal", "Value": "FilterValue"}] </param>
        /// <param name="searchTerm">To searching data.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <param name="pageSize">The page size.</param>
        /// <param name="sortField">The entity's field name to sort.</param>
        /// <param name="sortOrder">The sort order asc or desc.</param>
        /// <returns>The filtered list of counters</returns>/// <exception cref="Exception"></exception>
        public List<Counter> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            var result = GetCounter(filters, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return result;
        }

        /// <summary>Adds a new counter</summary>
        /// <param name="model">The counter data to be added</param>
        /// <returns>The result of the operation</returns>
        public Guid Create(Counter model)
        {
            model.Id = CreateCounter(model);
            return model.Id;
        }

        /// <summary>Updates a specific counter by its primary key</summary>
        /// <param name="id">The primary key of the counter</param>
        /// <param name="updatedEntity">The counter data to be updated</param>
        /// <returns>The result of the operation</returns>
        /// <exception cref="Exception"></exception>
        public bool Update(Guid id, Counter updatedEntity)
        {
            UpdateCounter(id, updatedEntity);
            return true;
        }

        /// <summary>Updates a specific counter by its primary key</summary>
        /// <param name="id">The primary key of the counter</param>
        /// <param name="updatedEntity">The counter data to be updated</param>
        /// <returns>The result of the operation</returns>
        /// <exception cref="Exception"></exception>
        public bool Patch(Guid id, JsonPatchDocument<Counter> updatedEntity)
        {
            PatchCounter(id, updatedEntity);
            return true;
        }

        /// <summary>Deletes a specific counter by its primary key</summary>
        /// <param name="id">The primary key of the counter</param>
        /// <returns>The result of the operation</returns>
        /// <exception cref="Exception"></exception>
        public bool Delete(Guid id)
        {
            DeleteCounter(id);
            return true;
        }
        #region
        private List<Counter> GetCounter(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            if (pageSize < 1)
            {
                throw new ApplicationException("Page size invalid!");
            }

            if (pageNumber < 1)
            {
                throw new ApplicationException("Page mumber invalid!");
            }

            var query = _dbContext.Counter.AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Counter>.ApplyFilter(query, filters, searchTerm);
            if (!string.IsNullOrEmpty(sortField))
            {
                var parameter = Expression.Parameter(typeof(Counter), "b");
                var property = Expression.Property(parameter, sortField);
                var lambda = Expression.Lambda<Func<Counter, object>>(Expression.Convert(property, typeof(object)), parameter);
                if (sortOrder.Equals("asc", StringComparison.OrdinalIgnoreCase))
                {
                    result = result.OrderBy(lambda);
                }
                else if (sortOrder.Equals("desc", StringComparison.OrdinalIgnoreCase))
                {
                    result = result.OrderByDescending(lambda);
                }
                else
                {
                    throw new ApplicationException("Invalid sort order. Use 'asc' or 'desc'");
                }
            }

            var paginatedResult = result.Skip(skip).Take(pageSize).ToList();
            return paginatedResult;
        }

        private Guid CreateCounter(Counter model)
        {
            _dbContext.Counter.Add(model);
            _dbContext.SaveChanges();
            return model.Id;
        }

        private void UpdateCounter(Guid id, Counter updatedEntity)
        {
            _dbContext.Counter.Update(updatedEntity);
            _dbContext.SaveChanges();
        }

        private bool DeleteCounter(Guid id)
        {
            var entityData = _dbContext.Counter.FirstOrDefault(entity => entity.Id == id);
            if (entityData == null)
            {
                throw new ApplicationException("No data found!");
            }

            _dbContext.Counter.Remove(entityData);
            _dbContext.SaveChanges();
            return true;
        }

        private void PatchCounter(Guid id, JsonPatchDocument<Counter> updatedEntity)
        {
            if (updatedEntity == null)
            {
                throw new ApplicationException("Patch document is missing!");
            }

            var existingEntity = _dbContext.Counter.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
            {
                throw new ApplicationException("No data found!");
            }

            updatedEntity.ApplyTo(existingEntity);
            _dbContext.Counter.Update(existingEntity);
            _dbContext.SaveChanges();
        }
        #endregion
    }
}